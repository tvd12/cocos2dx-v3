...
void TdgServerExtensionListener::processUserInfoData(boost::shared_ptr<ISFSObject> userInfoData)
{
    ...
    SERVER_ROOM().fetchingRoomStaticInfo(roomStatics);
    
    // update room dynamic
    Room *lobbyRoom = SERVER().getSmartFox()->LastJoinedRoom().get();
    SERVER_ROOM().fetchingRoomDynamicInfo(lobbyRoom);
    ...
    bool *isTheFirstTime;
    isTheFirstTime = userInfoData->GetBool(EX_REQUEST_PARAM_USER_FIRST_JOIN_GAME).get();
