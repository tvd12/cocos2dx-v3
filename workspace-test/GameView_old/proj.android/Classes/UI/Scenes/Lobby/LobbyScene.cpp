...
void LobbyLayer::addLabels()
{
    ...
    mTotalChipsLabel = CCLabelTTF::create(CURRENCY_LONG(mDisplayTotalChips),
                                          DEFAULT_FONT_NAME,
    ...
    if (GPARAMS().isIpadDevice) {
        CCLabelTTF *fullname = CCLabelTTF::create(USERINFO()->getFullName(),
                                                  DEFAULT_FONT_NAME,
                                                  GPARAMS().CMM_FONT_SIZE_2);
        fullname->setAnchorPoint(ccp(0, 0.5f));
        fullname->setPosition(GPARAMS().LBB_FOOTER_FULLNAME_POSITION);
        this->addChild(fullname, FOOTER_ORDER);
    }
}
...
void LobbyLayer::addMenuItems()
{
    ...
    mBonusChipsLabel = CCLabelTTF::create(CURRENCY_LONG(HOURLYBONUS()->getCurrentBonus()),
                                          DEFAULT_FONT_NAME,
                                          GPARAMS().CMM_FONT_SIZE_3);
//    mBonusChipsLabel->setAnchorPoint(ccp(0, 0.5f));
//    mBonusChipsLabel->setPosition(GPARAMS().LBB_MIDDLE_BONUS_LABEL_POSITION);
//    this->addChild(mBonusChipsLabel, FOOTER_ORDER);
...
void LobbyLayer::update(float dt)
{
    ...
    if (mIsJustGotDailyBonus) {
        mIsJustGotDailyBonus = false;
        mIsNeedUpdateTotalChips = true;

        SOUND().playSound(SOUND_COLLECT_CHIPS);
    }

    // update total chip if have
    updateTotalChipsLabel();
}

void LobbyLayer::updateTotalChipsLabel()
{
    if (mIsNeedUpdateTotalChips == false) {
        return;
    }
    
    if (USERINFO()->getTotalChips() > mDisplayTotalChips) {
        // Update displayValue
        long long deltaValue = llabs(USERINFO()->getTotalChips() - mDisplayTotalChips);
        if (deltaValue < 6) {
            mDisplayTotalChips += deltaValue;
        }
        else {
            deltaValue = (deltaValue / JACKPOT_DISPLAY_UPDATE_RATE);
            
            if (deltaValue <= 1) {
                mDisplayTotalChips += deltaValue + 1;
            }
            else {
                mDisplayTotalChips += arc4random() % deltaValue + 1;
            }
        }
        if (mDisplayTotalChips > USERINFO()->getTotalChips()) {
            mDisplayTotalChips = USERINFO()->getTotalChips();
        }
        
        // Update to label
        mTotalChipsLabel->setString(CURRENCY_LONG(mDisplayTotalChips));
    }
    else if (USERINFO()->getTotalChips() < mDisplayTotalChips) {
        // Update displayValue
        long long deltaValue = llabs(USERINFO()->getTotalChips() - mDisplayTotalChips);
        if (deltaValue < 6) {
            mDisplayTotalChips -= deltaValue;
            ...
            mDisplayTotalChips = (deltaValue / JACKPOT_DISPLAY_UPDATE_RATE);
...
void LobbyLayer::selectAvatarCallback(const char* memeberID)
{
    ...
    ProfilePopupView* profilePopup = ProfilePopupView::create();
