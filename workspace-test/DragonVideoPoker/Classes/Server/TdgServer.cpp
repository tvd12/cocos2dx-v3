...
TdgServer::TdgServer()
{
    ...
    mLeaderboard = Leaderboard::create();
    mLeaderboard->retain();
...
void TdgServer::login()
{
    ...
    CCLOG("********** Server URL: %s:%ld", mSmartFox->CurrentIp()->c_str(),
          mSmartFox->CurrentPort());
...
void TdgServer::sendUpdateFirstNameRequest(const std::string &pFirstName)
{
    CCLOG("new name is: %s", pFirstName.c_str());
    boost::shared_ptr<ISFSObject> parameters (new SFSObject());
    parameters->PutUtfString(EX_REQUEST_NAME_UPDATE_FIRSTNAME,
                             pFirstName);
    
	boost::shared_ptr<IRequest> request (new ExtensionRequest(EX_REQUEST_NAME_UPDATE,
                                                              parameters));
	mSmartFox->Send(request);
}

void TdgServer::sendUpdateLeaderboardRequest() {
    boost::shared_ptr<ISFSObject> parameters (new SFSObject());
	boost::shared_ptr<IRequest> request (new ExtensionRequest(EX_REQUEST_GET_LEADER_BOARD,
                                                              parameters));
	mSmartFox->Send(request);
}
