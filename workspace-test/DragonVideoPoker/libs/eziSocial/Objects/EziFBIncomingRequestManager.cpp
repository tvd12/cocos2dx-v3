...
bool EziFBIncomingRequestManager::removeIncomingRequestIDs(std::string requestIDs)
{
    // Get the user IDs into vector.
    std::vector< std::string > incomingRequestVector = EziSocialObject::tokenizeStr(requestIDs.c_str(), ",");
    int totalIncomingRequest = incomingRequestVector.size();
    
    bool removedAnyNewIncomingRequest = false;
...
//        }
        ...
        if (needToRemove) // This mean request ID is not added to pending list or completed list. It's time to add it in pending list.
        {
            _pendingRequestDictionary->removeObjectForKey(incomingRequestID);
            removedAnyNewIncomingRequest = true;
        }
    }
    
    if (removedAnyNewIncomingRequest)
    {
        this->saveUpdatedRequestList();
    }
    
    return removedAnyNewIncomingRequest;
}
...
{
    if (_pendingRequestDictionary == NULL) {
        return 0;
    }
    else {
