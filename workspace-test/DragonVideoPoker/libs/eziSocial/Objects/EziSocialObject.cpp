        ...
        responseCode == EziSocialWrapperNS::RESPONSE_CODE::FB_INVITE_RECEIVE)
    {
        ...
        switch (responseCode)
        {
            ...
            case EziSocialWrapperNS::RESPONSE_CODE::FB_INVITE_RECEIVE:
                EziFBIncomingRequestManager::sharedManager()->removeIncomingRequestIDs(requestID);
                break;
                
            // Only Gift will be saved
            case EziSocialWrapperNS::RESPONSE_CODE::FB_GIFT_RECEIVE:
            {
                EziFBIncomingRequest* incomingRequest =
                EziFBIncomingRequestManager::sharedManager()->getIncomingRequest(requestID);
...
{
    this->getFriends(searchType, 0, 5000);
}
...
void EziSocialObject::downloadPhoto(const char *fbID, const char* filename, bool forceDownloadFromServer)
{
    ...
    request->setResponseCallback((CCNode*)mFacebookDelegate, httpresponse_selector(EziSocialObject::onHttpRequestCompleted));
